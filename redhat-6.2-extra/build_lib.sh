...
OS_BASIC_PACKAGES=(MAKEDEV upstart audit-libs basesystem bash binutils bzip2-libs chkconfig cracklib cracklib-dicts crontabs coreutils db4 device-mapper e2fsprogs e2fsprogs-libs elfutils-libelf ethtool expat file-libs filesystem findutils gawk gdbm glib2 glibc glibc-common grep info initscripts iputils keyutils-libs krb5-libs libacl libattr libcap libcom_err libgcc libidn libselinux libsepol libstdc++ libsysfs libgcrypt libnih dbus-libs libcurl curl lua compat-libtermcap libutempter libxml2 libxml2-python logrotate m2crypto mcstrans mingetty mlocate module-init-tools ncurses ncurses-libs neon net-tools nss nss-sysinit nss-softokn nss-softokn-freebl openldap libssh2 cyrus-sasl-lib nss-util nspr openssl pam passwd libuser pcre popt procps psmisc python python-libs python-pycurl python-iniparse python-urlgrabber readline rpm rpm-libs rpm-python sed setup shadow-utils redhat-release-server-6Server sqlite rsyslog tzdata udev util-linux-ng xz xz-libs yum yum-plugin-downloadonly yum-metadata-parser yum-utils zlib)
...
__make_chroot() {
	...
	for f in "$IMAGE_DIR/Packages/$pkg"-[0-9]*+(noarch|x86_64).rpm; do
	    rpm2cpio "$f" | \
...
shrink_iso() {
    ...
    in_chroot /bin/bash -c 'cd /mnt; createrepo -g repodata/comps-rhel6-server-core.xml .'
