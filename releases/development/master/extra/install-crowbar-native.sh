...
if [[ -f /etc/redhat-release || -f /etc/centos-release ]]; then
    OS=redhat
    yum -y install ruby ruby-devel libxml2-devel zlib-devel gcc make postgresql93 \
    postgresql93-server postgresql93-devel postgresql93-contrib libpqxx
    PGSVCNAME=postgresql-9.3
    PG_DIR=/var/lib/pgsql/9.3/data
    service ${PGSVCNAME} initdb
    sed -i 's/5432/5439/g' /etc/init.d/postgresql-9.3
